...
export function listRecipes(options: { userId?: string; q?: string; categoryId?: string; favoritesOnly?: boolean } = {}) {
  ...
  const rows = db
    .prepare(`SELECT r.id, r.title, r.description, r.cover_image_url as coverImageUrl, r.category_id as categoryId,
      c.name as categoryName, r.difficulty, r.prep_time_minutes as prepTimeMinutes, r.cook_time_minutes as cookTimeMinutes,
...
export function saveRecipe(input: {
  ...
  const tx = db.transaction(() => {
    if (input.id) {
      db.prepare(`UPDATE recipes SET title=?, description=?, cover_image_url=COALESCE(?, cover_image_url), category_id=?, difficulty=?, prep_time_minutes=?, cook_time_minutes=?, servings=?, tips=?, updated_at=? WHERE id=?`)
        .run(input.title, input.description || null, input.coverImageUrl || null, input.categoryId || null, input.difficulty, input.prepTimeMinutes ?? null, input.cookTimeMinutes ?? null, input.servings ?? null, input.tips || null, now, id);
      db.prepare("DELETE FROM ingredients WHERE recipe_id = ?").run(id);
      db.prepare("DELETE FROM recipe_steps WHERE recipe_id = ?").run(id);
    } else {
      db.prepare(`INSERT INTO recipes (id, title, description, cover_image_url, category_id, difficulty, prep_time_minutes, cook_time_minutes, servings, tips, created_by_id, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, input.title, input.description || null, input.coverImageUrl || null, input.categoryId || null, input.difficulty, input.prepTimeMinutes ?? null, input.cookTimeMinutes ?? null, input.servings ?? null, input.tips || null, input.userId, now);
    }
